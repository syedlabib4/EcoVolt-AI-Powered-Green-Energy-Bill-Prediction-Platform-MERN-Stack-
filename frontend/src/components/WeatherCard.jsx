  ...
  useEffect(() => {
    const fetchWeather = async () => {
      ...
      try {
        ...
        const url = `https://api.openweathermap.org/data/2.5/weather?lat=24.8607&lon=67.0011&appid=${apiKey}&units=metric`;
        ...
        setWeather({
          temp: data.main.temp || DUMMY_WEATHER.temp,
          humidity: data.main.humidity || DUMMY_WEATHER.humidity,
          wind: data.wind?.speed || DUMMY_WEATHER.wind,
          condition: data.weather?.[0]?.main || "Live",
        });
        setIsLive(true);
      } catch (err) {
